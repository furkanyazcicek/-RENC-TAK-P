...
const OUT = path.join(ROOT, 'docs/ai-koc/kanitlar/faz-7')
const ACCEPTANCE = path.join(OUT, 'faz-7-kabul-sonucu.json')
...
const PHASE_ACCEPTANCE = Object.fromEntries([3, 4, 5, 6].map((phase) => [
...
function authoritative(phase, value) {
  ...
  return value.status === 'passed'
    ...
    && value.gate_totals?.required === 9 && value.gate_totals?.passed === 9
    && value.assertions === 107
}

function validateCore(phase, core) {
  if (phase === 3) return core?.status === 'core_passed_runner_pending'
    && core?.assertions === 450
  ...
  return core?.status === 'core_passed_runner_pending' && core?.assertions === 107 && core?.gate_totals?.passed === 9
}
...
async function main() {
  ...
  const preamble = `# DRKOÇ Faz 7 zorunlu doğrulama kaydı\ngenerated_at=${iso()}\nworkspace=${ROOT}\nrunner=scripts/run-phase-7-regressions.mjs\n`
  ...
  for (const entry of REQUIRED) {
    ...
    if (entry.key === 'phase7') {
      ...
      if (core?.status !== 'core_passed_runner_pending' || core?.assertions !== 137 || core?.gate_totals?.required !== 11 || core?.gate_totals?.passed !== 11) {
        result.status = 'failed'
        result.reason ??= 'Faz 7 çekirdek koşusu 137 doğrulama ve 11/11 kapı üretmedi.'
      }
    }
    results.push(result)
  }
  ...
  await writeFile(COMMANDS, `${JSON.stringify({
    schema_version: 'phase-7-regression-commands@1',
    generated_at: iso(),
    workspace: ROOT,
    runner: 'scripts/run-phase-7-regressions.mjs',
    totals,
  ...
  acceptance.status = allCommands
    && acceptance.gate_totals?.required === 11 && acceptance.gate_totals?.passed === 11
  ...
  process.stdout.write(`\n${totals.passed}/${totals.required} komut, ${acceptance.gate_totals.passed}/11 Faz 7 kapısı geçti.\n`)
