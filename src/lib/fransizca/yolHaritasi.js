 ...
 *   5. Fransızca düşünme antrenmanı
 *
...
export const GOREV_TURLERI = {
  tekrar: { ad: 'Kelime tekrarı', rota: '/fransizca/tekrar' },
  ders: { ad: 'Ders', rota: '/fransizca/ders' },
  destek: { ad: 'Destek çalışması', rota: '/fransizca/ders' },
  dusunme: { ad: 'Fransızca düşünme', rota: '/fransizca/dusunme' },
  telaffuz: { ad: 'Telaffuz', rota: '/fransizca/telaffuz' },
  degerlendirme: { ad: 'Seviye sonu değerlendirme', rota: '/fransizca/kutuphane' },
}
...
export function sonrakiDers(ilerleme, dersler) {
  ...
  /* Gelen liste zaten müfredat sırasındadır (bkz. content/fransizca/index.js).
...
export function bugunPlani(ilerleme, kaynak) {
  ...
  if (yeniDers && !gorevler.some((g) => g.id === `ders-${yeniDers.id}`)) {
    ...
    ekle({
      ...
      rota: `/fransizca/ders/${yeniDers.id}`,
  ...
  if (zayif && dakika < yogunluk.gunlukDakika) {
    ...
    if (zayif === 'telaffuz') {
      ...
      ekle({
        id: 'dusunme-destek',
        tur: 'dusunme',
        baslik: 'Fransızca düşünme antrenmanı',
  ...
  if (dakika < yogunluk.gunlukDakika - 5 && kelimeler.length) {
    ekle({
      ...
      rota: '/fransizca/tekrar?mod=yeni',
      dakika: 8,
      oncelik: 5,
    })
  }
