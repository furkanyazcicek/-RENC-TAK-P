/**
 * DRKOÇ — FRANSIZCA SESLENDİRME KÖPRÜSÜ
 * ==================================================================
 *
 * DÜRÜSTLÜK KURALI: Bu bölümde sahte oynatıcı yoktur. Ses ya gerçekten
 * çıkar ya da öğrenciye "bu cihazda Fransızca ses bulunamadı" denir ve
 ...
 * Fransızca sesi kuruluysa kalitesi iyidir; kurulu değilse hiç ses çıkmaz.
 * Arayüz bu farkı gizlemez.
 *
 * BÖLGE: Fransızca yalnız Fransa'nın dili değildir; Kanada (Québec),
 * Belçika ve İsviçre'de de resmî dildir ve sesletim gerçekten farklıdır
 * (özellikle Québec Fransızcası). Cihazda o bölgenin sesi varsa kullanılır,
 * yoksa sessizce en yakın Fransızca sese düşülür — öğrenciye "Québec sesi
 * çalıyorum" denip Fransa sesi çalınmaz.
 *
 * VARSAYILAN: fr-FR. Öğrencinin karşılaşacağı sınav, film ve ders
 * kaynaklarının neredeyse tamamı Fransa Fransızcasıdır.
 *
...
const TERCIH_SIRASI = [
  // Tarayıcı ve işletim sistemlerinin en doğal Fransızca sesleri.
  'Thomas', 'Audrey', 'Aurelie', 'Aurélie', 'Marie', 'Amelie', 'Amélie',
  'Google français', 'Microsoft Denise', 'Microsoft Julie',
  'Microsoft Hortense', 'Microsoft Paul', 'Eddy (français (France))',
  'Flo (français (France))', 'Chantal', 'Nicolas',
]
...
/** Cihazda kullanılabilir Fransızca sesler. */
export function fransizcaSesler(hepsi = sesListesi) {
  return (hepsi ?? []).filter((s) => /^fr([-_]|$)/i.test(s.lang ?? ''))
}

/** Bölgeye göre en uygun sesi seçer; o bölge yoksa genel Fransızca sese düşer. */
function sesSec(bolge = 'fr') {
  const adaylar = fransizcaSesler()
  if (!adaylar.length) return null
  const desen = BOLGELER[bolge] ?? BOLGELER.fr
 ...
 * @param {'fr'|'ca'|'be'|'ch'} [secenek.bolge]
...
export async function sesDurumu() {
  if (!sesDesteginiKontrolEt()) {
    return {
      hazir: false,
      sebep: 'destek-yok',
      mesaj: 'Bu tarayıcı Fransızca seslendirmeyi desteklemiyor. Dinleme çalışmaları yazılı biçimde açılacak.',
    }
  }
  const sesler = await sesleriYukle()
  const fransizca = fransizcaSesler(sesler)
  if (!fransizca.length) {
    return {
      hazir: false,
      sebep: 'fransizca-ses-yok',
      mesaj: 'Cihazında kurulu bir Fransızca ses bulunamadı. Dinleme çalışmaları yazılı biçimde açılacak.',
      cozum: 'Telefonda Ayarlar → Erişilebilirlik → Konuşma bölümünden Fransızca (Français) ses indirebilirsin.',
    }
  }
  return {
    hazir: true,
    sesSayisi: fransizca.length,
    bolgeler: {
      fr: fransizca.some((s) => BOLGELER.fr.test(s.lang)),
      ca: fransizca.some((s) => BOLGELER.ca.test(s.lang)),
      be: fransizca.some((s) => BOLGELER.be.test(s.lang)),
      ch: fransizca.some((s) => BOLGELER.ch.test(s.lang)),
    },
  }
}
