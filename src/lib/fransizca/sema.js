 ...
 * KELİME KAYDI
 * ------------------------------------------------------------------
 *   { id, dil, kelime, tur, seviye, fransizcaTanim, turkceDestek, ipa,
 *     artikel?('le'|'la'|"l'"), cins?('eril'|'disil'), cogul?, cekim?,
 ...
 *
 *   FRANSIZCAYA ÖZGÜ ZORUNLULUK: `tur` 'isim' ise `artikel`, `cins` ve
 ...
 *
 *   `cins` NEDEN AYRI BİR ALAN: sesli harfle başlayan isimlerde artikel
 *   "l'" olur (l'école, l'hôtel) ve cinsiyeti GİZLER. Öğrenci yalnız
 *   artikele bakarak cinsiyeti öğrenemeyeceği için içerik bunu açıkça
 *   yazmak zorundadır. İçerik testi bunu arar.
 *
 * ------------------------------------------------------------------
 * PDF NOTU
 * ------------------------------------------------------------------
 ...
 * Ekranlar bunu çalıştırmaz; içerik testi (`npm run test:fransizca`) çalıştırır.
 */
...
export function dersDogrula(ders) {
  ...
  if (ders.dil !== 'fr') ek(`beklenmeyen dil: ${ders.dil}`)
...
export const ARTIKELLER = ['le', 'la', "l'"]
...
export function kelimeDogrula(k) {
  ...
  if (!k?.fransizcaTanim) ek('Fransızca tanım eksik — öğrenci dostu tanım zorunlu.')
  ...
  /* Fransızcanın kendi kuralı: isim = artikel + cinsiyet + çoğul.
     Bu üçlü eksikse kelime öğretilmiş sayılmaz. */
  if (k?.tur === 'isim') {
    if (!ARTIKELLER.includes(k?.artikel)) ek("isim için artikel (le/la/l') zorunlu.")
  ...
  if (k?.tur === 'fiil' && k?.duzensiz && !k?.cekim) {
    ek('düzensiz fiil için cekim alanı zorunlu (ör. "je vais · tu vas · nous allons").')
  }
  return h
}
...
export function desteDogrula(deste) {
  ...
  if (deste?.dil !== 'fr') ek(`beklenmeyen dil: ${deste?.dil}`)
