/**
 * DRKOÇ — FRANSIZCA İLERLEME DEPOSU
 * ==================================================================
 *
...
const ANAHTAR = 'drkoc-fransizca-v1'
const SURUM = 1
...
/**
 * Yamayı mevcut duruma yazar ve `fransizca-ilerleme` olayını yayar.
...
export function ilerlemeYaz(yama) {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('fransizca-ilerleme', { detail: yeni }))
  }
  return yeni
}
...
export function ilerlemeyiSil(onay) {
  if (onay !== 'FRANSIZCA-SIFIRLA') return false
  try {
    localStorage.removeItem(ANAHTAR)
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('fransizca-ilerleme', { detail: klon(BOS_ILERLEME) }))
    }
    return true
  } catch {
    return false
