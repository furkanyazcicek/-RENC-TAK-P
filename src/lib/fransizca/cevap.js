 ...
 *  • Aksanı klavyede bulamayan öğrenci cezalandırılmaz: "eleve" yazımı
 *    "élève" ile aynı sayılır. Türkiye'deki klavyede é/è/ê/à/ç yoktur;
 *    öğrenci bilmediği için değil, tuş olmadığı için yazamaz. Cevap doğru
 *    sayılır ve doğru yazım gösterilir.
 *  • Tire ile boşluk aynı sayılır: "parlez vous" = "parlez-vous". Tirenin
 *    yeri öğretilir ama cevap bu yüzden düşürülmez.
 *  • Fransızcada gün, ay ve dil adları KÜÇÜK harfle yazılır (lundi, janvier,
 *    le français). İngilizceden gelen alışkanlıkla büyük yazan öğrencinin
...
const YAZIM_ESITLEMELERI = [
  /* ELİZYON — Fransızcada zorunludur ve öğrenci klavyede kesme işaretini
     bazen boşlukla yazar: "j ai" ya da "j' ai". Anlam aynıdır, bitişik
     yazıma indirgenir. DİKKAT: "je ai" biçimi BURADA düzeltilmez; o bir
     yazım tercihi değil, elizyonu hiç yapmamaktır ve öğretilmesi gerekir
     (bkz. turkceIzleri.js → 'elizyon-yok'). */
  [/\b([cdjlmnst]|qu)'\s+/gi, "$1'"],
  [/\b([cdjlmnst]|qu)\s+'/gi, "$1'"],
  /* Kesme işaretini hiç yazmayan öğrenci: "j ai" → "j'ai". Fransızcada tek
     harflik bir sözcüğün ardından sesli/h gelmesi neredeyse her zaman
     elizyondur; bu yüzden bitişik yazıma indirgemek güvenlidir. */
  [/\b([cdjlmnst]|qu)\s+(?=[aeiouyâàéèêëîïôöùûü]|h)/gi, "$1'"],

  /* Soru kalıbının parçaları ayrı yazılmış olabilir: "est ce que". */
  [/\bqu'est\s*[- ]?\s*ce\s*[- ]?\s*que\b/gi, "qu'est-ce que"],
  [/\bqu'est\s*[- ]?\s*ce\s*[- ]?\s*qui\b/gi, "qu'est-ce qui"],
  [/\best\s*[- ]?\s*ce\s*[- ]?\s*que\b/gi, 'est-ce que'],
  [/\best\s*[- ]?\s*ce\s*[- ]?\s*qui\b/gi, 'est-ce qui'],

  /* Konuşma dilinde çok yaygın kısaltmalar. İçerik açık biçimi yazsa da
     öğrenci duyduğu gibi yazabilir; ikisi de aynı yere düşsün. */
  [/\bt'as\b/g, 'tu as'],
  [/\bt'es\b/g, 'tu es'],
  [/\by'a\b/g, 'il y a'],
]
 ...
 *
 * NEDEN: Türkiye'deki bir öğrencinin klavyesinde é/è/ê/à/ç/ù/î/ô yoktur.
 * Öğrenci "élève" kelimesini bilse bile "eleve" yazmak zorunda kalır.
 * Aksan farkı yüzünden cevabı yanlışa düşürmek, bilgiyi değil klavyeyi
 * ölçmek olur.
 *
 * BEDELİ VE ÇÖZÜMÜ: Fransızcada aksan bazen anlam ayırır (ou/où, a/à,
 * sur/sûr). Bu yüzden aksansız cevap doğru sayılır AMA her seferinde doğru
...
function sikiNormalize(metin) {
  /* normalize() aksanları zaten düşürdüğü için sade Latin harfleri yeterli.
     TİRE burada BOŞLUĞA çevrilir: "parlez-vous" ile "parlez vous" aynı
     cevaptır; tirenin yeri öğretilir ama cevap bu yüzden düşürülmez.
     Kesme işareti KORUNUR — elizyon Fransızcada dil bilgisidir, noktalama
     süsü değildir. */
...
/**
 * Fransızcada gün adları, ay adları ve dil adları KÜÇÜK harfle yazılır:
...
function kucukHarfNotu(ogrenciHam, dogruMetin) {
  ...
  return `Doğru. Yalnız şunu unutma: Fransızcada gün, ay ve dil adları küçük harfle yazılır — ${buyukYazilan.join(', ')}.`
}

/** Fark yalnızca eksik aksan mı? ("eleve" ↔ "élève") */
...
function yazimNotu(ham, dogru) {
  if (aksanFarkiMi(ham, dogru)) {
    return `Doğru. Aksana dikkat: doğru yazımı "${dogru}". Aksan Fransızcada bazen anlamı değiştirir (ou = veya · où = nerede).`
  }
  ...
  if (hamDuz !== dogruDuz && bicimsiz(ham) === bicimsiz(dogru)) {
    return `Doğru. Yazımı tam biçimiyle not et: "${dogru}". Fransızcada kesme işareti ve tire dil bilgisinin parçasıdır, süs değildir.`
  }
  return null
}
  ...
  for (const d of liste) {
    if (ogrenciSiki && ogrenciSiki === sikiNormalize(d)) {
      return {
        ...
        eslesen: d,
        not: yazimNotu(ham, d)
          ?? 'Doğru. Tire ve noktalamaya da dikkat edersen yazım tam olur (ör. "parlez-vous", "est-ce que").',
      }
    }
  }
