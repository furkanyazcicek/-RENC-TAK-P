 ...
 *   5. İngilizce düşünme antrenmanı
 *
...
export const GOREV_TURLERI = {
  tekrar: { ad: 'Kelime tekrarı', rota: '/ingilizce/tekrar' },
  ders: { ad: 'Ders', rota: '/ingilizce/ders' },
  destek: { ad: 'Destek çalışması', rota: '/ingilizce/ders' },
  dusunme: { ad: 'İngilizce düşünme', rota: '/ingilizce/dusunme' },
  telaffuz: { ad: 'Telaffuz', rota: '/ingilizce/telaffuz' },
  degerlendirme: { ad: 'Seviye sonu değerlendirme', rota: '/ingilizce/kutuphane' },
}
...
export function bugunPlani(ilerleme, kaynak) {
  ...
  if (yeniDers && !gorevler.some((g) => g.id === `ders-${yeniDers.id}`)) {
    ...
    ekle({
      ...
      rota: `/ingilizce/ders/${yeniDers.id}`,
  ...
  if (zayif && dakika < yogunluk.gunlukDakika) {
    ...
    if (zayif === 'telaffuz') {
      ...
      ekle({
        id: 'dusunme-destek',
        tur: 'dusunme',
        baslik: 'İngilizce düşünme antrenmanı',
  ...
  if (dakika < yogunluk.gunlukDakika - 5 && kelimeler.length) {
    ekle({
      ...
      rota: '/ingilizce/tekrar?mod=yeni',
      dakika: 8,
      oncelik: 5,
    })
  }
