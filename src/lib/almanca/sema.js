 ...
 *
 * DİL BAĞIMSIZ: `dil` alanı bilinçli olarak zorunlu. Bu ağaçta yalnızca 'de'
 ...
 *   { tur: 'ornek',        satirlar: [{ de, tr?, not? }] }
 *   { tur: 'kalip',        baslik, kaliplar: [{ de, kullanim, ornek }] }
 ...
 *   { tur: 'okuma',        baslik, metin, sozluk?: [{ de, tr }] }
 *   { tur: 'dinleme',      baslik, metin, satirlar?: [{ kisi?, de }], hiz? }
 *   { tur: 'ipucu',        metin }
 *
 * ------------------------------------------------------------------
 ...
 * KELİME KAYDI
 * ------------------------------------------------------------------
 *   { id, dil, kelime, tur, seviye, almancaTanim, turkceDestek, ipa,
 *     artikel?('der'|'die'|'das'), cogul?, cekim?,
 *     heceler, ornekler:[{de,tr}], esdizim:[], esAnlam:[], zitAnlam:[],
 *     aile:[], hata?, kayit?('resmi'|'gundelik'|'argo'),
 *     bolgesel?:{de?,at?,ch?}, hatirlatma?, uretim?:{yonerge,kabul:[]} }
 *
 *   ALMANCAYA ÖZGÜ ZORUNLULUK: `tur` 'isim' ise `artikel` ve `cogul`
 *   ZORUNLUDUR. Artikelsiz öğrenilen bir isim yarım öğrenilmiştir; öğrenci
 *   o kelimeyi hiçbir cümlede doğru kullanamaz. İçerik testi bunu arar.
 *
 * ------------------------------------------------------------------
 * PDF NOTU
 * ------------------------------------------------------------------
 ...
 * Ekranlar bunu çalıştırmaz; içerik testi (`npm run test:almanca`) çalıştırır.
 */
...
export function dersDogrula(ders) {
  ...
  if (ders.dil !== 'de') ek(`beklenmeyen dil: ${ders.dil}`)
...
export const ARTIKELLER = ['der', 'die', 'das']
...
export function kelimeDogrula(k) {
  ...
  if (!k?.almancaTanim) ek('Almanca tanım eksik — öğrenci dostu tanım zorunlu.')
  ...
  /* Almancanın kendi kuralı: isim = artikel + kelime + çoğul.
     Bu üçlü eksikse kelime öğretilmiş sayılmaz. */
  if (k?.tur === 'isim') {
    if (!ARTIKELLER.includes(k?.artikel)) ek('isim için artikel (der/die/das) zorunlu.')
  ...
  if (k?.tur === 'fiil' && k?.duzensiz && !k?.cekim) {
    ek('düzensiz fiil için cekim alanı zorunlu (ör. "fährt · ist gefahren").')
  }
  return h
}
...
export function desteDogrula(deste) {
  ...
  if (deste?.dil !== 'de') ek(`beklenmeyen dil: ${deste?.dil}`)
