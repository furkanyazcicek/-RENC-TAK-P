...
const ANAHTAR = 'drkoc-almanca-v1'
const SURUM = 1
...
/**
 * Yamayı mevcut duruma yazar ve `almanca-ilerleme` olayını yayar.
...
export function ilerlemeYaz(yama) {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('almanca-ilerleme', { detail: yeni }))
  }
  return yeni
}
...
export function ilerlemeyiSil(onay) {
  if (onay !== 'ALMANCA-SIFIRLA') return false
  try {
    localStorage.removeItem(ANAHTAR)
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('almanca-ilerleme', { detail: klon(BOS_ILERLEME) }))
    }
    return true
  } catch {
    return false
