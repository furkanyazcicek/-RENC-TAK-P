...
const ANAHTAR = 'drkoc-ispanyolca-v1'
const SURUM = 1
...
export function ilerlemeYaz(yama) {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('ispanyolca-ilerleme', { detail: yeni }))
  }
  return yeni
}
...
export function ilerlemeyiSil(onay) {
  ...
  try {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('ispanyolca-ilerleme', { detail: klon(BOS_ILERLEME) }))
    }
    return true
  } catch {
    return false
