...
export function useLessonAuth() {
  const auth = useAuth()
  const preview = isLessonPreview()
  const role = preview ? previewRole() : null

  /**
   * KARARLI NESNE ŞART: her render'da yeni bir `session`/`user` üretilirse,
   * bunları bağımlılık dizisine koyan bileşenler (davet ekranı gibi) sonsuz
   * döngüye giriyor ve "yükleniyor" ekranında takılı kalıyordu.
   */
  const previewAuth = useMemo(() => {
    const profile = role === 'student' ? STUDENT : TEACHER
    return {
      loading: false,
  ...
  if (!preview) return auth
  return { ...auth, ...previewAuth }
}
