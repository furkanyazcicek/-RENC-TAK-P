...
import { yazmaGeriBildirim } from '../../../lib/almanca/cevap'
        ...
        <div className="grid gap-4">
          ...
          {alistirma.ornekMetin && (
            <div className="rounded-input bg-surface-sunken px-4 py-4 ring-1 ring-inset ring-line">
              ...
              <p className="mt-2 whitespace-pre-line text-[0.95rem] leading-relaxed text-ink" lang="de">
                {alistirma.ornekMetin}
              </p>
            </div>
          )}
      ...
      {adim === 2 && (
        <div className="grid gap-4">
          ...
          <Textarea
            ...
            rows={9}
            lang="de"
            placeholder="Schreib deinen Text auf Deutsch…"
...
function SurumKutusu({ baslik, metin, sayac, vurgulu = false }) {
  return (
    ...
    >
      <p className="text-2xs font-bold uppercase tracking-[0.14em] text-ink/45">{baslik}</p>
      <p className="mt-1.5 whitespace-pre-line text-sm leading-relaxed text-ink" lang="de">{metin}</p>
      {sayac && (
