...
import { alistirmaKontrol, DURUM } from '../../../lib/fransizca/cevap'
 ...
 *
 * NEDEN TEK BİLEŞEN: doğruluk kararı her tür için `lib/fransizca/cevap.js`
...
function Govde({ alistirma, cevap, setCevap, kilitli, sesHazir, sonuc, kontrolEt }) {
  switch (alistirma.tur) {
    ...
    case 'hata-bul':
      return (
        <>
          <p className="rounded-input bg-surface-sunken px-4 py-3 font-body text-base text-ink" lang="fr">
            {alistirma.cumle}
          </p>
      ...
      return (
        <>
          ...
          <MetinCevap
            ...
            onEnter={kontrolEt}
            cokSatir
            yerTutucu="Fransızca ne söylersin?"
          />
        </>
      )
      ...
      return (
        <>
          <div className="rounded-input bg-surface-sunken px-4 py-3">
            ...
            <p className="mt-1 text-base leading-relaxed text-ink" lang="fr">{alistirma.tanim}</p>
            {alistirma.ilkHarf && (
              <p className="mt-2 text-sm text-ink/55">
      ...
      return (
        <>
          <div className="flex flex-wrap items-center gap-3 rounded-input bg-surface-sunken px-4 py-3">
            <p className="min-w-0 flex-1 text-base leading-relaxed text-ink" lang="fr">{alistirma.soru}</p>
  ...
  if (!sesHazir) {
    ...
    return (
      <div className="mb-4 rounded-input bg-warning-500/[0.07] px-4 py-3 ring-1 ring-inset ring-warning-500/20">
        ...
        <p className="mt-1 text-base leading-relaxed text-ink" lang="fr">{metin}</p>
        <p className="mt-1.5 text-xs text-ink/55">
          Cihazında Fransızca ses bulunamadı, bu yüzden metni okuyarak çözüyorsun.
        </p>
      </div>
    )
  }
...
function Secenekler({ soru, secenekler, secili, onSec, kilitli, dogruId, fransizca = false }) {
  return (
    <>
      {soru && (
        <p
          className={cn('mb-3 text-base leading-relaxed text-ink', fransizca && 'text-ink/70')}
          lang={fransizca ? undefined : 'en'}
        >
          {soru}
        </p>
      )}
        ...
        {secenekler.map((s, sira) => {
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0 flex-1 text-[0.95rem] leading-relaxed text-ink" lang="fr">
                {s.metin}
              </span>
            </button>
          )
...
function Bosluk({ alistirma, cevap, setCevap, kilitli, detay }) {
  ...
  return (
    <>
      ...
      <p className="whitespace-pre-line text-base leading-loose text-ink" lang="fr">
