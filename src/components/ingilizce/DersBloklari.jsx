 ...
 *
 * İngilizce metinlere `lang="en"` verilir: ekran okuyucular Türkçe
 * seslendirmeyle İngilizce cümle okumaz.
 */
        ...
        <section>
          ...
          <div className="grid gap-2.5">
            {blok.satirlar.map((s, i) => (
              ...
              >
                <p className="min-w-0 flex-1 text-[0.98rem] font-medium leading-relaxed text-ink" lang="en">
                  {s.en}
                </p>
                <SesDugmesi metin={s.en} hazir={sesHazir} boyut="sm" />
            ...
            {blok.kaliplar.map((k, i) => (
              <div key={i} className="rounded-input bg-aqua-500/[0.06] px-4 py-3 ring-1 ring-inset ring-aqua-500/12">
                <div className="flex flex-wrap items-center gap-3">
                  <p className="min-w-0 flex-1 font-display text-[1.02rem] font-semibold text-ink" lang="en">
                    {k.en}
                  </p>
                  <SesDugmesi metin={k.en} hazir={sesHazir} boyut="sm" />
                </div>
                <p className="mt-1 text-sm text-ink/60">{k.kullanim}</p>
                {k.ornek && (
                  <p className="mt-1.5 text-sm italic leading-relaxed text-ink/70" lang="en">{k.ornek}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <section className="rounded-card bg-surface-muted p-5 ring-1 ring-inset ring-line">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div className="rounded-input bg-success-500/[0.08] px-4 py-3 ring-1 ring-inset ring-success-500/18">
              ...
              <p className="mt-1 text-[0.95rem] leading-relaxed text-ink" lang="en">{blok.dogal}</p>
            </div>
            <div className="rounded-input bg-ink/[0.04] px-4 py-3 ring-1 ring-inset ring-ink/[0.06]">
              ...
              <p className="mt-1 text-[0.95rem] leading-relaxed text-ink/65" lang="en">{blok.yapay}</p>
            </div>
          </div>
                ...
                <div className="mt-2.5 flex flex-wrap items-center gap-2">
                  {ses.kelimeler.map((k) => (
                    <span key={k} className="inline-flex items-center gap-1.5">
                      <span className="rounded-md bg-surface px-2.5 py-1 text-sm font-medium text-ink ring-1 ring-inset ring-line" lang="en">
                        {k}
                      </span>
...
function OkumaBlogu({ blok, sesHazir }) {
  ...
  return (
    <section className="rounded-card border border-line bg-surface p-5 shadow-card">
      ...
      <p className="mt-3 whitespace-pre-line text-[1.02rem] leading-[1.8] text-ink" lang="en">
        {blok.metin}
      </p>
        ...
        <div className="mt-4 border-t border-line pt-3">
          ...
          {sozlukAcik && (
            <dl className="mt-2 grid gap-1.5 sm:grid-cols-2">
              {blok.sozluk.map((s) => (
                <div key={s.en} className="flex items-baseline gap-2 rounded-md bg-surface-sunken px-3 py-2">
                  <dt className="font-semibold text-ink" lang="en">{s.en}</dt>
      ...
      {sesHazir ? (
        <>
          <div className="mt-3 flex flex-wrap items-center gap-2.5">
            ...
            <SesDugmesi metin={blok.metin} hazir etiket="Yavaş dinle" aksan="ing" yavasSecenegi={false} />
          </div>
        ...
        <p className="mt-2 rounded-input bg-warning-500/[0.07] px-3.5 py-2.5 text-sm leading-relaxed text-ink/70 ring-1 ring-inset ring-warning-500/20">
          Cihazında İngilizce ses bulunamadı. Bu çalışmayı metni okuyarak yapabilirsin; dinleme
        ...
        <div className="mt-3 grid gap-2">
          {(blok.satirlar ?? []).map((satir, i) => (
            <div key={i} className="flex flex-wrap items-center gap-2.5 rounded-input bg-surface-sunken px-3.5 py-2.5">
              ...
              <p className="min-w-0 flex-1 text-[0.98rem] leading-relaxed text-ink" lang="en">{satir.en}</p>
              <SesDugmesi metin={satir.en} hazir={sesHazir} boyut="sm" />
            </div>
          ))}
          {!blok.satirlar?.length && (
            <p className="whitespace-pre-line text-[0.98rem] leading-relaxed text-ink" lang="en">{blok.metin}</p>
          )}
        </div>
      )}
    </section>
