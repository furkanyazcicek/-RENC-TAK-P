...
import { alistirmaKontrol, DURUM } from '../../../lib/ingilizce/cevap'
 ...
 *
 * NEDEN TEK BİLEŞEN: doğruluk kararı her tür için `lib/ingilizce/cevap.js`
...
function Govde({ alistirma, cevap, setCevap, kilitli, sesHazir, sonuc, kontrolEt }) {
  switch (alistirma.tur) {
    ...
    case 'hata-bul':
      return (
        <>
          <p className="rounded-input bg-surface-sunken px-4 py-3 font-body text-base text-ink" lang="en">
            {alistirma.cumle}
          </p>
      ...
      return (
        <>
          ...
          <MetinCevap
            ...
            onEnter={kontrolEt}
            cokSatir
            yerTutucu="İngilizce ne söylersin?"
          />
        </>
      )
      ...
      return (
        <>
          <div className="rounded-input bg-surface-sunken px-4 py-3">
            ...
            <p className="mt-1 text-base leading-relaxed text-ink" lang="en">{alistirma.tanim}</p>
            {alistirma.ilkHarf && (
              <p className="mt-2 text-sm text-ink/55">
      ...
      return (
        <>
          <div className="flex flex-wrap items-center gap-3 rounded-input bg-surface-sunken px-4 py-3">
            <p className="min-w-0 flex-1 text-base leading-relaxed text-ink" lang="en">{alistirma.soru}</p>
            <SesDugmesi metin={alistirma.soru} hazir={sesHazir} boyut="sm" />
          </div>
          <MetinCevap value={cevap} onChange={setCevap} kilitli={kilitli} onEnter={kontrolEt} cokSatir yerTutucu="Answer in English…" />
        </>
      )
  ...
  if (!sesHazir) {
    ...
    return (
      <div className="mb-4 rounded-input bg-warning-500/[0.07] px-4 py-3 ring-1 ring-inset ring-warning-500/20">
        ...
        <p className="mt-1 text-base leading-relaxed text-ink" lang="en">{metin}</p>
        <p className="mt-1.5 text-xs text-ink/55">
          Cihazında İngilizce ses bulunamadı, bu yüzden metni okuyarak çözüyorsun.
        </p>
      </div>
    )
  }
...
function Secenekler({ soru, secenekler, secili, onSec, kilitli, dogruId, ingilizce = false }) {
  return (
    <>
      {soru && (
        <p
          className={cn('mb-3 text-base leading-relaxed text-ink', ingilizce && 'text-ink/70')}
          lang={ingilizce ? undefined : 'en'}
        >
          {soru}
        </p>
      )}
        ...
        {secenekler.map((s, sira) => {
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0 flex-1 text-[0.95rem] leading-relaxed text-ink" lang="en">
                {s.metin}
              </span>
            </button>
          )
...
function Bosluk({ alistirma, cevap, setCevap, kilitli, detay }) {
  ...
  return (
    <>
      ...
      <p className="whitespace-pre-line text-base leading-loose text-ink" lang="en">
