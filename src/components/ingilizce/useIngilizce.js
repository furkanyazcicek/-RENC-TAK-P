...
import { ilerlemeOku } from '../../lib/ingilizce/ilerleme'
import { sesDurumu } from '../../lib/ingilizce/ses'

/**
 * useIlerleme — öğrencinin İngilizce ilerlemesini okur ve değiştiğinde
 * kendini tazeler.
 *
 * Ekranlar `localStorage`'a dokunmaz; ilerleme deposu bir olay yayar
 * (`ingilizce-ilerleme`) ve bütün ekranlar aynı anda güncellenir. Bu
...
export function useIlerleme() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('ingilizce-ilerleme', tazele)
    // Başka bir sekmede çalışıldıysa bu sekme de haberdar olsun.
    window.addEventListener('storage', tazele)
    return () => {
      window.removeEventListener('ingilizce-ilerleme', tazele)
...
/**
 * useSes — cihazda İngilizce ses olup olmadığını bir kez sorar.
 *
