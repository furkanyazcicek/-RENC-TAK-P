...
import Alistirma from '../../components/ingilizce/alistirmalar/Alistirma'
import SesDugmesi from '../../components/ingilizce/SesDugmesi'
import { useIlerleme, useSes } from '../../components/ingilizce/useIngilizce'
import { KELIME_HAVUZU } from '../../content/ingilizce'
import { DURUM } from '../../lib/ingilizce/cevap'
import { dilEtkinligiKaydet, favoriDegistir, gunlukKaydet, ilerlemeOku, ilerlemeYaz } from '../../lib/ingilizce/ilerleme'
import {
  BOYUTLAR, kartCevapla, kartOzeti, sonrakiBoyut, tekrarOturumu,
} from '../../lib/ingilizce/tekrar'
import { DURUM_ETIKETLERI } from '../../lib/ingilizce/ilerleme'
 ...
 *
 *   tanıma      → kelimeyi görünce anlamını biliyor mu (İngilizce tanım)
...
export default function Tekrar() {
  ...
  const cevapla = (sonuc) => {
    ...
    dilEtkinligiKaydet({
      activityType: 'review_result_snapshot', activityId: `tekrar:${suanki.kelime.id}:${boyut}`,
      skillDomain: boyut, correctCount: dogruMu ? 1 : 0, incorrectCount: dogruMu ? 0 : 1,
      blankCount: 0, helpCount: dogruMu ? 0 : 1, reviewOutcome: dogruMu ? 'recalled' : 'forgotten',
      completionStatus: 'answered', cefrLevel: suanki.kelime.seviye,
    })
    ...
    return (
      <AppShell title="Kelime tekrarı" subtitle="Bugün için bekleyen kart yok" width="narrow">
        <EmptyState
          ...
          action={<Button as={Link} to="/ingilizce">Ana sayfaya dön</Button>}
        />
        <KartOzeti ozet={ozet} />
      </AppShell>
    )
    ...
    return (
      <AppShell title="Kelime tekrarı" subtitle="Oturum tamamlandı" width="narrow">
        ...
        <div className="flex flex-col gap-2.5 sm:flex-row">
          ...
          <Button as={Link} to="/ingilizce" variant="secondary" className="justify-center">
            Ana sayfaya dön
          </Button>
        </div>
      </AppShell>
...
function kartAlistirmasi({ kelime, kart }, sesHazir) {
  ...
  if (boyut === 'tanima') {
    ...
    const secenekler = karistir([
      { id: 'dogru', metin: kelime.ingilizceTanim },
      ...yanlislar.map((k, i) => ({ id: `y${i}`, metin: k.ingilizceTanim })),
    ])
    return {
  ...
  if (boyut === 'baglam') {
    ...
    return {
      ...
      secenekler,
      dogruId: 'dogru',
      aciklama: kelime.hata ?? `${kelime.kelime}: ${kelime.ingilizceTanim}`,
    }
  }
  ...
  if (boyut === 'tamamlama') {
    ...
    const parcalar = ornek.en.split(desen)
    return {
      ...
      parcalar: parcalar.length > 1
        ...
        : [`${ornek.tr} → `, { bosluk: 0 }],
      cevaplar: [{
        kabul: [kelime.kelime, kelime.kelime.split(' ')[0], ...(desen.exec(ornek.en) ? [desen.exec(ornek.en)[0]] : [])],
    ...
    <section className="rounded-card border border-line bg-surface p-5 shadow-card">
      ...
      <div className="mt-3.5 grid gap-2">
        ...
        {kelime.ornekler.map((o) => (
          <div key={o.en} className="flex flex-wrap items-center gap-2.5 rounded-input bg-surface-muted px-3.5 py-2.5">
            <p className="min-w-0 flex-1 text-[0.95rem] leading-relaxed text-ink" lang="en">{o.en}</p>
            <SesDugmesi metin={o.en} hazir={sesHazir} boyut="sm" />
        ...
        <div className="mt-3.5">
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {kelime.esdizim.map((e) => (
              <span key={e} className="rounded-full bg-aqua-500/10 px-2.5 py-1 text-xs font-medium text-aqua-700 ring-1 ring-inset ring-aqua-500/15" lang="en">
                {e}
              </span>
            ))}
          </div>
      ...
      {kelime.ingUsa && (
        <p className="mt-3 rounded-input bg-info-500/[0.06] px-3.5 py-2.5 text-sm leading-relaxed text-ink/70 ring-1 ring-inset ring-info-500/15">
          <strong className="font-semibold text-ink">İngiliz / Amerikan farkı: </strong>
          ABD {kelime.ingUsa.usa} · İngiltere {kelime.ingUsa.ing}
        </p>
      )}
...
function Satir({ etiket, deger, ingilizce = false }) {
  return (
    <div className="flex flex-col gap-0.5 sm:flex-row sm:gap-3">
      <dt className="w-40 shrink-0 text-2xs font-bold uppercase tracking-[0.1em] text-ink/45">{etiket}</dt>
      <dd className="min-w-0 flex-1 text-[0.95rem] leading-relaxed text-ink/80" lang={ingilizce ? 'en' : undefined}>
        {deger}
      </dd>
    </div>
  )
...
function Etiketler({ baslik, liste }) {
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink/70" lang="en">{liste.join(', ')}</p>
    </div>
  )
}
