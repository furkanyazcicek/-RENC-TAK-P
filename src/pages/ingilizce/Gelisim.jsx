...
import { BECERILER, SEVIYELER } from '../../lib/ingilizce/seviyeler'
import { bekleyenSayisi, kartOzeti } from '../../lib/ingilizce/tekrar'
import { izBul } from '../../lib/ingilizce/turkceIzleri'
import { baslangicSeviyesi, seviyeTamamlanmaDurumu } from '../../lib/ingilizce/yolHaritasi'
...
export default function Gelisim() {
  ...
  const disaAktar = () => {
    ...
    bag.href = adres
    bag.download = `drkoc-ingilizce-ilerleme-${new Date().toISOString().slice(0, 10)}.json`
    bag.click()
  ...
  return (
    <AppShell title="Gelişimim" subtitle={`${seviye} · ${SEVIYELER[seviye]?.ad ?? ''}`}>
      ...
      <section className="rounded-card border border-line bg-surface p-5 shadow-card">
        ...
        <Button as={Link} to="/ingilizce/tekrar" variant="secondary" size="sm" className="mt-3">
      ...
      <section className="rounded-card bg-surface-muted p-5 ring-1 ring-inset ring-line">
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-ink/65">
          İngilizce ilerlemen şu an <strong className="font-semibold text-ink">yalnızca bu
      ...
      <Modal open={silmeAcik} onClose={() => setSilmeAcik(false)} title="İlerlemeyi sıfırla">
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          ...
          <Button
            variant="danger"
            onClick={() => { ilerlemeyiSil('INGILIZCE-SIFIRLA'); setSilmeAcik(false) }}
          >
            Evet, sıfırla
          </Button>
        </div>
