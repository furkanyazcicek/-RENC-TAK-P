...
import { useSes } from '../../components/ingilizce/useIngilizce'
import { SESLER, SHADOWING } from '../../content/ingilizce'
import { kayitBaslat, kaydiBirak, kayitDesteginiKontrolEt } from '../../lib/ingilizce/kayit'
    ...
    <>
      ...
      <section className="rounded-card bg-surface-muted p-5 ring-1 ring-inset ring-line">
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-ink/65">
          ...
          alfabesinden gelir. Ezberlemene gerek yok — İngilizce yazıldığı gibi okunmadığı için, bir
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {ses.kelimeler.map((k) => (
            <span key={k} className="inline-flex items-center gap-1.5 rounded-input bg-surface px-3 py-2 ring-1 ring-inset ring-line">
              <span className="text-[0.95rem] font-medium text-ink" lang="en">{k}</span>
            ...
            <div key={`${c.a}-${c.b}`} className="rounded-input bg-surface-muted px-4 py-3">
              <div className="flex flex-wrap items-center gap-3">
                <span className="inline-flex items-center gap-1.5">
                  <span className="font-display text-base font-bold text-ink" lang="en">{c.a}</span>
                  <SesDugmesi metin={c.a.split(' ')[0]} hazir={sesHazir} boyut="sm" />
                </span>
                <span className="text-ink/30" aria-hidden="true">↔</span>
                <span className="inline-flex items-center gap-1.5">
                  <span className="font-display text-base font-bold text-ink" lang="en">{c.b}</span>
      ...
      <section className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-card bg-surface-muted p-5 ring-1 ring-inset ring-line">
          ...
          <p className="mt-1.5 text-[0.98rem] leading-relaxed text-ink" lang="en">{ses.cumle}</p>
        ...
        <div className="rounded-card bg-aqua-500/[0.07] p-5 ring-1 ring-inset ring-aqua-500/18">
          ...
          <p className="mt-1.5 text-[0.98rem] leading-relaxed text-ink" lang="en">{ses.dogalKonusma}</p>
          ...
          <div className="mt-3 grid gap-2">
            {p.satirlar.map((s) => (
              <div key={s} className="flex flex-wrap items-center gap-2.5 rounded-input bg-surface-muted px-3.5 py-2.5">
                <p className="min-w-0 flex-1 text-[0.98rem] leading-relaxed text-ink" lang="en">{s}</p>
    ...
    <section className="rounded-card border border-line bg-surface p-5 shadow-card">
      ...
      {hedefMetin && (
        <div className="mt-3 flex flex-wrap items-center gap-2.5 rounded-input bg-surface-muted px-4 py-3">
          <p className="min-w-0 flex-1 text-[0.98rem] leading-relaxed text-ink" lang="en">{hedefMetin}</p>
