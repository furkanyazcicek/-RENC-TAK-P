...
import DersBloklari from '../../components/ingilizce/DersBloklari'
import Alistirma from '../../components/ingilizce/alistirmalar/Alistirma'
import YazmaGorevi from '../../components/ingilizce/alistirmalar/YazmaGorevi'
import { useIlerleme, useSes } from '../../components/ingilizce/useIngilizce'
import SesDugmesi, { SesUyarisi } from '../../components/ingilizce/SesDugmesi'
import DersKartlari from '../../components/dil/DersKartlari'
import { dersBul, dersKelimeleri, modulDestesi, notBul } from '../../content/ingilizce'
...
} from '../../lib/ingilizce/ilerleme'
import { BECERILER } from '../../lib/ingilizce/seviyeler'
 ...
 * oranına göre belirlenir (bkz. lib/ingilizce/ilerleme.js `dersTamamla`).
 */
...
export default function Ders() {
  ...
  const cevapAlindi = (s) => {
    ...
    dilEtkinligiKaydet({
      activityType: 'lesson_result_snapshot', activityId: `${ders.id}:${asama}:${etkinlik?.id ?? 'bilinmeyen'}`,
      contentRevision: String(ders.surum ?? 1), skillDomain: etkinlik?.beceri ?? ders.odakBeceri ?? 'general',
      correctCount: dogruMu ? 1 : 0, incorrectCount: dogruMu ? 0 : 1, blankCount: 0,
      helpCount: dogruMu ? 0 : 1, completionStatus: 'answered', cefrLevel: ders.seviye,
    })
    skorRef.current = {
  ...
  return (
    <AppShell
      ...
      width="narrow"
      headerAction={
        <Button as={Link} to="/ingilizce/kutuphane" variant="ghost" size="sm">
          Kütüphane
        </Button>
      }
    >
          ...
          {alistirmalar[alistirmaIndeks]?.tur === 'yazma' ? (
            <YazmaGorevi
              ...
              onTamamla={({ ilkSurum, sonSurum }) => {
                ...
                dilEtkinligiKaydet({
                  activityType: 'self_report_snapshot', activityId: `${ders.id}:alistirma:${alistirmalar[alistirmaIndeks].id}`,
                  contentRevision: String(ders.surum ?? 1), skillDomain: 'yazma', completionStatus: 'completed', cefrLevel: ders.seviye,
                })
      ...
      {asama === 'bitis' && sonuc && (
        <Bitis
          ...
          onSonraki={() => {
            if (ders.sonraki) {
              yonlendir(`/ingilizce/ders/${ders.sonraki}`)
              sifirla()
            } else {
              yonlendir('/ingilizce')
            }
    ...
    <>
      ...
      {izler.length > 0 && (
        <section className="rounded-card border border-line bg-surface p-5 shadow-card">
          ...
          <Button as={Link} to="/ingilizce/dusunme" variant="secondary" size="sm" className="mt-3">
