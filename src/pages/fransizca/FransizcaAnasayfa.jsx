...
import {
  ...
  Layers, LibraryBig, LineChart, Mic, RefreshCw, Sparkles, Target,
} from 'lucide-react'
...
import { baslangicTamamMi, seriHesapla, tespitTamamMi } from '../../lib/fransizca/ilerleme'
import { SEVIYELER } from '../../lib/fransizca/seviyeler'
import { bekleyenSayisi } from '../../lib/fransizca/tekrar'
import { baslangicSeviyesi, bugunPlani, seviyeTamamlanmaDurumu } from '../../lib/fransizca/yolHaritasi'

/**
 * Fransızca ana sayfası.
 *
...
export default function FransizcaAnasayfa() {
...
function Tanitim() {
  return (
    ...
    >
      <DashboardHero
        asPageHeader
        eyebrow="Fransızca Öğrenme Kütüphanesi"
      ...
      <section className="rounded-card bg-surface-muted p-5 ring-1 ring-inset ring-line sm:p-6">
        ...
        <p className="mt-2 text-[0.95rem] leading-relaxed text-ink/70">
          ...
          durum verir ve o durumda Fransızca ne söylendiğini sorar. Yanlış yaptığında da yalnız
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <Button as={Link} to="/fransizca/baslangic" size="lg" icon={Sparkles} className="justify-center">
          Başlayalım
        </Button>
        <Button as={Link} to="/fransizca/kilavuz" size="lg" variant="secondary" className="justify-center">
...
function TespitBekliyor({ ilerleme }) {
  ...
  return (
    <AppShell title="Fransızca" subtitle="Sıradaki adım: seviye tespiti" width="narrow" showPageIntro={false}>
      <DashboardHero
        asPageHeader
      ...
      <div className="rounded-card border border-line bg-surface p-5 shadow-card sm:p-6">
        ...
        <ul className="mt-3 grid gap-2 sm:grid-cols-2">
          {[
            ...
            'Okuduğunu anlama',
            'Dinlediğini anlama (cihazında Fransızca ses varsa)',
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <Button as={Link} to="/fransizca/seviye-tespit" size="lg" icon={Target} className="justify-center">
          Seviye tespitine başla
        </Button>
        <Button as={Link} to="/fransizca/kutuphane" size="lg" variant="secondary" className="justify-center">
          Sınavsız devam et
        </Button>
      </div>
    </AppShell>
...
function GunlukAkis({ ilerleme }) {
  ...
  return (
    <AppShell
      ...
      headerAction={
        <Button as={Link} to="/fransizca/gelisim" variant="ghost" size="sm" icon={LineChart}>
          Gelişimim
        </Button>
      }
      showPageIntro={false}
    >
      ...
      <section>
        ...
        {gorevler.length === 0 ? (
          <div className="mt-3 rounded-card border border-line bg-surface p-6 text-center shadow-card">
            ...
            <Button as={Link} to="/fransizca/kutuphane" variant="secondary" className="mt-4">
              Kütüphaneye git
            </Button>
          </div>
        ) : (
      ...
      <section>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {[
            { to: '/fransizca/kutuphane', Icon: LibraryBig, ad: 'Ders kütüphanesi', not: 'Seviyelere ve modüllere göre bütün dersler' },
            { to: '/fransizca/yol-haritasi', Icon: CalendarCheck, ad: 'Yol haritam', not: 'Haftalık plan, 30 günlük hedef, kontrol noktaları' },
            { to: '/fransizca/tekrar', Icon: RefreshCw, ad: 'Kelime ve tekrar', not: 'Aralıklı tekrar — unutmadan önce hatırla' },
            { to: '/fransizca/kartlar', Icon: Layers, ad: 'Dil kartları', not: 'İlgi alanına göre desteler — çevir, karşılığını ve örneği gör' },
            { to: '/fransizca/telaffuz', Icon: Mic, ad: 'Telaffuz laboratuvarı', not: 'Türkçede olmayan sesler, vurgu, ritim, kendi kaydın' },
            { to: '/fransizca/dusunme', Icon: BrainCircuit, ad: 'Fransızca düşünme', not: 'Çeviri alışkanlığını kıran antrenmanlar' },
            { to: '/fransizca/kilavuz', Icon: Compass, ad: 'Çalışma kılavuzu', not: 'Bu bölüm nasıl kullanılır, nereden başlanır' },
          ].map(({ to, Icon, ad, not }) => (
            <Link
              key={to}
              to={to}
