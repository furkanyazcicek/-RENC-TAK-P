...
import { DERSLER } from '../../content/fransizca'
import { BECERILER, SEVIYELER, SEVIYE_SIRASI, seviyeIndeksi } from '../../lib/fransizca/seviyeler'
import { izBul } from '../../lib/fransizca/turkceIzleri'
import { haftalikPlan, otuzGunHedefi, sonrakiDers } from '../../lib/fransizca/yolHaritasi'
...
export default function Sonuc() {
  ...
  if (!sonuc) return <Navigate to="/fransizca" replace />
    ...
    <AppShell title="Seviye tespiti sonucu" subtitle="Nerede olduğun ve nereden devam edeceğin">
      ...
      {sonuc.izler.length > 0 && (
        <section className="rounded-card border border-line bg-surface p-5 shadow-card">
          ...
          <Button as={Link} to="/fransizca/dusunme" variant="secondary" className="mt-4" iconRight={ArrowRight}>
            Fransızca düşünme antrenmanına git
          </Button>
        </section>
      )}
      ...
      {ilkDers && (
        <section className="rounded-card bg-brand-500/[0.07] p-5 ring-1 ring-inset ring-brand-500/18 sm:p-6">
          ...
          <Button as={Link} to={`/fransizca/ders/${ilkDers.id}`} className="mt-4" iconRight={ArrowRight}>
            Derse başla
          </Button>
        </section>
      )}
      ...
      <div className="flex flex-col gap-2.5 sm:flex-row">
        <Button as={Link} to="/fransizca" size="lg" className="justify-center" iconRight={ArrowRight}>
          Çalışmaya başla
        </Button>
        <Button as={Link} to="/fransizca/yol-haritasi" size="lg" variant="secondary" className="justify-center">
