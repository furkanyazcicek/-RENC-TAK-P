...
import { DERSLER, KELIME_HAVUZU, modulBul } from '../../content/fransizca'
import { DURUMLAR, DURUM_ETIKETLERI } from '../../lib/fransizca/ilerleme'
...
} from '../../lib/fransizca/yolHaritasi'
      ...
      {zayif && (
        <section className="rounded-card bg-info-500/[0.06] p-5 ring-1 ring-inset ring-info-500/18">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {zayif === 'telaffuz' && (
              <Button as={Link} to="/fransizca/telaffuz" variant="secondary" size="sm">Telaffuz laboratuvarı</Button>
            )}
            {(zayif === 'dusunme' || zayif === 'dizim') && (
              <Button as={Link} to="/fransizca/dusunme" variant="secondary" size="sm">Fransızca düşünme antrenmanı</Button>
            )}
            {(zayif === 'kelime') && (
              <Button as={Link} to="/fransizca/tekrar" variant="secondary" size="sm">Kelime tekrarı</Button>
            )}
          </div>
        </section>
      )}
