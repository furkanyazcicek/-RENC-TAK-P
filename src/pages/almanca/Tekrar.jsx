...
import { DURUM_ETIKETLERI } from '../../lib/almanca/ilerleme'
 ...
 *
 *   tanıma      → kelimeyi görünce anlamını biliyor mu (Almanca tanım)
    ...
    return (
      <AppShell title="Kelime tekrarı" subtitle="Bugün için bekleyen kart yok" width="narrow">
        <EmptyState
          ...
          action={<Button as={Link} to="/almanca">Ana sayfaya dön</Button>}
        />
        <KartOzeti ozet={ozet} />
      </AppShell>
    )
    ...
    return (
      <AppShell title="Kelime tekrarı" subtitle="Oturum tamamlandı" width="narrow">
        ...
        <div className="flex flex-col gap-2.5 sm:flex-row">
          ...
          <Button as={Link} to="/almanca" variant="secondary" className="justify-center">
            Ana sayfaya dön
          </Button>
        </div>
      </AppShell>
...
function kartAlistirmasi({ kelime, kart }, sesHazir) {
  ...
  if (boyut === 'tanima') {
    ...
    const secenekler = karistir([
      { id: 'dogru', metin: kelime.almancaTanim },
      ...yanlislar.map((k, i) => ({ id: `y${i}`, metin: k.almancaTanim })),
    ])
    return {
  ...
  if (boyut === 'baglam') {
    ...
    return {
      ...
      secenekler,
      dogruId: 'dogru',
      aciklama: kelime.hata ?? `${kelime.kelime}: ${kelime.almancaTanim}`,
    }
  }
  ...
  if (boyut === 'tamamlama') {
    ...
    const parcalar = ornek.de.split(desen)
    return {
      ...
      parcalar: parcalar.length > 1
        ...
        : [`${ornek.tr} → `, { bosluk: 0 }],
      cevaplar: [{
        kabul: [kelime.kelime, kelime.kelime.split(' ')[0], ...(desen.exec(ornek.de) ? [desen.exec(ornek.de)[0]] : [])],
...
/**
 * Artikel rengi bir SÜS değil, hafıza kancasıdır: öğrenci der/die/das'ı
 * kelimeyle birlikte renk olarak da hatırlar. Renkler tasarım sisteminin
 * mevcut tonlarından seçildi; yeni renk uydurulmadı.
 */
const ARTIKEL_RENGI = {
  der: 'text-info-700',
  die: 'text-accent-700',
  das: 'text-success-700',
}
  ...
  return (
    <section className="rounded-card border border-line bg-surface p-5 shadow-card">
      <div className="flex flex-wrap items-center gap-3">
        {/* ARTİKEL BAŞLIKTA: Almancada bir ismi "bilmek", artikelini de
            bilmek demektir. Bu yüzden der/die/das kelimenin yanında değil,
            kelimeyle BİRLİKTE tek bir bütün olarak gösterilir. */}
        <h2 className="font-display text-xl font-bold text-ink" lang="de">
          {kelime.artikel && (
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {kelime.cogul && (
            <span className="rounded-full bg-brand-500/[0.08] px-3 py-1 text-sm text-ink/75 ring-1 ring-inset ring-brand-500/15">
              ...
              <span lang="de">{kelime.cogul}</span>
            </span>
          )}
          {kelime.cekim && (
            <span className="rounded-full bg-aqua-500/[0.08] px-3 py-1 text-sm text-ink/75 ring-1 ring-inset ring-aqua-500/15">
              ...
              <span lang="de">{kelime.cekim}</span>
            </span>
          )}
        </div>
      )}
      ...
      <dl className="mt-3.5 grid gap-2.5">
        <Satir etiket="Almanca tanım" deger={kelime.almancaTanim} almanca />
      ...
      <div className="mt-3.5 grid gap-2">
        ...
        {kelime.ornekler.map((o) => (
          <div key={o.de} className="flex flex-wrap items-center gap-2.5 rounded-input bg-surface-muted px-3.5 py-2.5">
            <p className="min-w-0 flex-1 text-[0.95rem] leading-relaxed text-ink" lang="de">{o.de}</p>
            <SesDugmesi metin={o.de} hazir={sesHazir} boyut="sm" />
        ...
        <div className="mt-3.5">
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {kelime.esdizim.map((e) => (
              <span key={e} className="rounded-full bg-aqua-500/10 px-2.5 py-1 text-xs font-medium text-aqua-700 ring-1 ring-inset ring-aqua-500/15" lang="de">
                {e}
              </span>
            ))}
          </div>
      ...
      {kelime.bolgesel && (
        <p className="mt-3 rounded-input bg-info-500/[0.06] px-3.5 py-2.5 text-sm leading-relaxed text-ink/70 ring-1 ring-inset ring-info-500/15">
          ...
          {[
            kelime.bolgesel.de && `Almanya ${kelime.bolgesel.de}`,
            kelime.bolgesel.at && `Avusturya ${kelime.bolgesel.at}`,
...
function Satir({ etiket, deger, almanca = false }) {
  return (
    <div className="flex flex-col gap-0.5 sm:flex-row sm:gap-3">
      <dt className="w-40 shrink-0 text-2xs font-bold uppercase tracking-[0.1em] text-ink/45">{etiket}</dt>
      <dd className="min-w-0 flex-1 text-[0.95rem] leading-relaxed text-ink/80" lang={almanca ? 'en' : undefined}>
        {deger}
      </dd>
    </div>
  )
...
function Etiketler({ baslik, liste }) {
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink/70" lang="de">{liste.join(', ')}</p>
    </div>
  )
}
