...
import { notBul } from '../../content/almanca'
import '../../styles/almanca-yazdir.css'
 ...
 * `.de-yazdir-gizle` sınıfıyla işaretlenir ve yazdırırken kaybolur.
 *
 * İKİ SÜRÜM:
 ...
 * eklenmez. Gerekçesi: src/styles/almanca-yazdir.css başındaki not.
 */
export default function Yazdir() {
  ...
  useEffect(() => {
    if (not) document.title = `${not.baslik} — DrKoç Almanca`
  ...
  return (
    <div className="min-h-screen bg-paper py-6 px-4 sm:px-6">
      ...
      <div className="de-yazdir-gizle mx-auto mb-5 flex max-w-[190mm] flex-wrap items-center gap-3">
        <Button as={Link} to="/almanca/kutuphane" variant="ghost" size="sm" icon={ArrowLeft}>
          Kütüphane
        </Button>
        <Tabs
          items={[
      ...
      <p className="de-yazdir-gizle mx-auto mb-5 max-w-[190mm] rounded-card bg-info-500/[0.06] px-4 py-3 text-sm leading-relaxed text-ink/70 ring-1 ring-inset ring-info-500/15">
        {ogretmenMi
      ...
      {/* Belgenin kendisi */}
      <article className={cn('de-yazdir', ogretmenMi && 'de-yazdir--ogretmen')}>
        <header className="de-yazdir__ust">
          <div>
            <span className="de-yazdir__marka">Dr. Koç · Almanca</span>
            <h1>{not.baslik}</h1>
            <p className="de-yazdir__altbaslik">{not.altBaslik}</p>
          </div>
              ...
              <tbody>
                {not.cevapAnahtari.map((c, i) => (
                  <tr key={i}>
                    ...
                    <td className="de-almanca">{c.cevap}</td>
                    <td>{c.aciklama ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
      ...
      return (
        <section>
          ...
          {bolum.maddeler?.length > 0 && (
            <ul>{bolum.maddeler.map((m) => <li key={m} className="de-almanca">{m}</li>)}</ul>
          )}
        </section>
      )
          ...
          <table>
            ...
            <tbody>
              {bolum.satirlar.map((satir, i) => (
                <tr key={i}>
                  {satir.map((h, j) => (
                    <td key={j} className={j === 0 ? 'de-almanca' : undefined}>{h}</td>
                  ))}
                </tr>
              ))}
            </tbody>
      ...
      return (
        <section>
          ...
          {bolum.satirlar.map((s) => (
            <p key={s.de}>
              <span className="de-almanca">{s.de}</span>
          ...
          <table>
            ...
            <tbody>
              {bolum.kelimeler.map((k) => (
                <tr key={k.de}>
                  <td className="de-almanca">{k.de}</td>
                  <td>{k.ipa}</td>
                  <td>{k.tr}</td>
                </tr>
              ))}
        ...
        <section>
          ...
          {bolum.satirlar.map((s) => (
            <div key={s.yanlis} className="de-kutu de-kutu--hata">
              <p style={{ margin: 0 }}>
                <strong>Yanlış: </strong><span className="de-almanca">{s.yanlis}</span>
              </p>
              <p style={{ margin: '2pt 0 0' }}>
                <strong>Doğru: </strong><span className="de-almanca">{s.dogru}</span>
              </p>
          ...
          {bolum.sorular.map((s) => (
            <div key={s.no} className="de-soru">
              ...
              <span className="de-soru__govde">
                <span className="de-almanca">{s.metin}</span>
