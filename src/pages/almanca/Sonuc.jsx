...
import { DERSLER } from '../../content/almanca'
...
import { haftalikPlan, otuzGunHedefi, sonrakiDers } from '../../lib/almanca/yolHaritasi'
...
export default function Sonuc() {
  ...
  if (!sonuc) return <Navigate to="/almanca" replace />
    ...
    <AppShell title="Seviye tespiti sonucu" subtitle="Nerede olduğun ve nereden devam edeceğin">
      ...
      {sonuc.izler.length > 0 && (
        <section className="rounded-card border border-line bg-surface p-5 shadow-card">
          ...
          <Button as={Link} to="/almanca/dusunme" variant="secondary" className="mt-4" iconRight={ArrowRight}>
            Almanca düşünme antrenmanına git
          </Button>
        </section>
      )}
      ...
      {ilkDers && (
        <section className="rounded-card bg-brand-500/[0.07] p-5 ring-1 ring-inset ring-brand-500/18 sm:p-6">
          ...
          <Button as={Link} to={`/almanca/ders/${ilkDers.id}`} className="mt-4" iconRight={ArrowRight}>
            Derse başla
          </Button>
        </section>
      )}
      ...
      <div className="flex flex-col gap-2.5 sm:flex-row">
        <Button as={Link} to="/almanca" size="lg" className="justify-center" iconRight={ArrowRight}>
          Çalışmaya başla
        </Button>
        <Button as={Link} to="/almanca/yol-haritasi" size="lg" variant="secondary" className="justify-center">
