...
import { BECERILER, SEVIYELER } from '../../lib/almanca/seviyeler'
import { bekleyenSayisi, kartOzeti } from '../../lib/almanca/tekrar'
import { izBul } from '../../lib/almanca/turkceIzleri'
import { baslangicSeviyesi, seviyeTamamlanmaDurumu } from '../../lib/almanca/yolHaritasi'
...
export default function Gelisim() {
  ...
  const disaAktar = () => {
    ...
    bag.href = adres
    bag.download = `drkoc-almanca-ilerleme-${new Date().toISOString().slice(0, 10)}.json`
    bag.click()
  ...
  return (
    <AppShell title="Gelişimim" subtitle={`${seviye} · ${SEVIYELER[seviye]?.ad ?? ''}`}>
      ...
      <section className="rounded-card border border-line bg-surface p-5 shadow-card">
        ...
        <Button as={Link} to="/almanca/tekrar" variant="secondary" size="sm" className="mt-3">
      ...
      <section className="rounded-card bg-surface-muted p-5 ring-1 ring-inset ring-line">
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-ink/65">
          Almanca ilerlemen şu an <strong className="font-semibold text-ink">yalnızca bu
      ...
      <Modal open={silmeAcik} onClose={() => setSilmeAcik(false)} title="İlerlemeyi sıfırla">
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          ...
          <Button
            variant="danger"
            onClick={() => { ilerlemeyiSil('ALMANCA-SIFIRLA'); setSilmeAcik(false) }}
          >
            Evet, sıfırla
          </Button>
        </div>
