...
import { KELIME_HAVUZU } from '../../content/ispanyolca'
import { DURUM } from '../../lib/ispanyolca/cevap'
import { dilEtkinligiKaydet, favoriDegistir, gunlukKaydet, ilerlemeOku, ilerlemeYaz } from '../../lib/ispanyolca/ilerleme'
import {
  BOYUTLAR, kartCevapla, kartOzeti, sonrakiBoyut, tekrarOturumu,
} from '../../lib/ispanyolca/tekrar'
import { DURUM_ETIKETLERI } from '../../lib/ispanyolca/ilerleme'
 ...
 *
 *   tanıma      → kelimeyi görünce anlamını biliyor mu (İspanyolca tanım)
    ...
    return (
      <AppShell title="Kelime tekrarı" subtitle="Bugün için bekleyen kart yok" width="narrow">
        <EmptyState
          ...
          action={<Button as={Link} to="/ispanyolca">Ana sayfaya dön</Button>}
        />
        <KartOzeti ozet={ozet} />
      </AppShell>
    )
    ...
    return (
      <AppShell title="Kelime tekrarı" subtitle="Oturum tamamlandı" width="narrow">
        ...
        <div className="flex flex-col gap-2.5 sm:flex-row">
          ...
          <Button as={Link} to="/ispanyolca" variant="secondary" className="justify-center">
            Ana sayfaya dön
          </Button>
        </div>
      </AppShell>
...
function kartAlistirmasi({ kelime, kart }, sesHazir) {
  ...
  if (boyut === 'tanima') {
    ...
    const secenekler = karistir([
      { id: 'dogru', metin: kelime.ispanyolcaTanim },
      ...yanlislar.map((k, i) => ({ id: `y${i}`, metin: k.ispanyolcaTanim })),
    ])
    return {
  ...
  if (boyut === 'baglam') {
    ...
    return {
      ...
      secenekler,
      dogruId: 'dogru',
      aciklama: kelime.hata ?? `${kelime.kelime}: ${kelime.ispanyolcaTanim}`,
    }
  }
  ...
  if (boyut === 'tamamlama') {
    ...
    const parcalar = ornek.es.split(desen)
    return {
      ...
      parcalar: parcalar.length > 1
        ...
        : [`${ornek.tr} → `, { bosluk: 0 }],
      cevaplar: [{
        kabul: [kelime.kelime, kelime.kelime.split(' ')[0], ...(desen.exec(ornek.es) ? [desen.exec(ornek.es)[0]] : [])],
 ...
 * DİŞİL mi olduğunu kelimeyle birlikte renk olarak da hatırlar. İspanyolcada
  ...
  return (
    <section className="rounded-card border border-line bg-surface p-5 shadow-card">
      <div className="flex flex-wrap items-center gap-3">
        {/* ARTİKEL BAŞLIKTA: İspanyolcada bir ismi "bilmek", cinsiyetini de
            bilmek demektir. Bu yüzden le/la kelimenin yanında değil,
            kelimeyle BİRLİKTE tek bir bütün olarak gösterilir. */}
        <h2 className="font-display text-xl font-bold text-ink" lang="es">
          {kelime.artikel && (
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {kelime.cogul && (
            <span className="rounded-full bg-brand-500/[0.08] px-3 py-1 text-sm text-ink/75 ring-1 ring-inset ring-brand-500/15">
              ...
              <span lang="es">{kelime.cogul}</span>
            </span>
          )}
          {kelime.cekim && (
            <span className="rounded-full bg-aqua-500/[0.08] px-3 py-1 text-sm text-ink/75 ring-1 ring-inset ring-aqua-500/15">
              ...
              <span lang="es">{kelime.cekim}</span>
            </span>
          )}
        </div>
      )}
      ...
      <dl className="mt-3.5 grid gap-2.5">
        <Satir etiket="İspanyolca tanım" deger={kelime.ispanyolcaTanim} ispanyolca />
      ...
      <div className="mt-3.5 grid gap-2">
        ...
        {kelime.ornekler.map((o) => (
          <div key={o.es} className="flex flex-wrap items-center gap-2.5 rounded-input bg-surface-muted px-3.5 py-2.5">
            <p className="min-w-0 flex-1 text-[0.95rem] leading-relaxed text-ink" lang="es">{o.es}</p>
            <SesDugmesi metin={o.es} hazir={sesHazir} boyut="sm" />
        ...
        <div className="mt-3.5">
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {kelime.esdizim.map((e) => (
              <span key={e} className="rounded-full bg-aqua-500/10 px-2.5 py-1 text-xs font-medium text-aqua-700 ring-1 ring-inset ring-aqua-500/15" lang="es">
                {e}
              </span>
            ))}
          </div>
      ...
      {kelime.bolgesel && (
        <p className="mt-3 rounded-input bg-info-500/[0.06] px-3.5 py-2.5 text-sm leading-relaxed text-ink/70 ring-1 ring-inset ring-info-500/15">
          ...
          {[
            kelime.bolgesel.es && `Fransa ${kelime.bolgesel.es}`,
            kelime.bolgesel.mx && `Québec ${kelime.bolgesel.mx}`,
...
function Satir({ etiket, deger, ispanyolca = false }) {
  return (
    <div className="flex flex-col gap-0.5 sm:flex-row sm:gap-3">
      <dt className="w-40 shrink-0 text-2xs font-bold uppercase tracking-[0.1em] text-ink/45">{etiket}</dt>
      <dd className="min-w-0 flex-1 text-[0.95rem] leading-relaxed text-ink/80" lang={ispanyolca ? 'en' : undefined}>
        {deger}
      </dd>
    </div>
  )
...
function Etiketler({ baslik, liste }) {
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink/70" lang="es">{liste.join(', ')}</p>
    </div>
  )
}
