...
import { DERSLER } from '../../content/ispanyolca'
import { BECERILER, SEVIYELER, SEVIYE_SIRASI, seviyeIndeksi } from '../../lib/ispanyolca/seviyeler'
import { izBul } from '../../lib/ispanyolca/turkceIzleri'
import { haftalikPlan, otuzGunHedefi, sonrakiDers } from '../../lib/ispanyolca/yolHaritasi'
...
export default function Sonuc() {
  ...
  if (!sonuc) return <Navigate to="/ispanyolca" replace />
    ...
    <AppShell title="Seviye tespiti sonucu" subtitle="Nerede olduğun ve nereden devam edeceğin">
      ...
      {sonuc.izler.length > 0 && (
        <section className="rounded-card border border-line bg-surface p-5 shadow-card">
          ...
          <Button as={Link} to="/ispanyolca/dusunme" variant="secondary" className="mt-4" iconRight={ArrowRight}>
            İspanyolca düşünme antrenmanına git
          </Button>
        </section>
      )}
      ...
      {ilkDers && (
        <section className="rounded-card bg-brand-500/[0.07] p-5 ring-1 ring-inset ring-brand-500/18 sm:p-6">
          ...
          <Button as={Link} to={`/ispanyolca/ders/${ilkDers.id}`} className="mt-4" iconRight={ArrowRight}>
            Derse başla
          </Button>
        </section>
      )}
      ...
      <div className="flex flex-col gap-2.5 sm:flex-row">
        <Button as={Link} to="/ispanyolca" size="lg" className="justify-center" iconRight={ArrowRight}>
          Çalışmaya başla
        </Button>
        <Button as={Link} to="/ispanyolca/yol-haritasi" size="lg" variant="secondary" className="justify-center">
